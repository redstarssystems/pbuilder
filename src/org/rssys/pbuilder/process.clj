...
(defn- add-extra-stuff->pom
  ...
  (with-open [rdr (io/reader (io/file "pom.xml"))]
    (let [pom (-> rdr
                parse-xml)
          pom (if url
      ...
      (spit "pom.xml" (str/replace (xml/indent-str pom) #"\n\s+\n" "\n"))
      (spit "pom.xml" "<!-- This file was autogenerated by build tool pbuilder.
...
(defn- deps-content []
  (deps-reader/slurp-deps (io/file "deps.edn")))
...
(defn detect-class-conflicts
  ...
  * Returns:
  ...
  ([]
   (detect-class-conflicts {:deps-map (deps-reader/slurp-deps (io/file "deps.edn"))}))
  ...
  (let [pom-props (pom/make-pom-properties (symbol group-artifact-id) {:mvn/version artifact-version})
    ...
    (uberjar/bundle out-path
      {;; A map with the same format than deps.edn. :deps-map is used to resolve the project resources.
       :deps-map                    (deps-reader/slurp-deps (io/file "deps.edn"))
