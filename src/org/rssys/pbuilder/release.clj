...
(defn run-release
  ...
  (if (assert-commited?)
    (do
      (println "ok.")
      (let [_               (println "changing artefact version...")
            ...
            _               (when-not (zero? (Long/parseLong (:out result)))
            ...
            _               (println "building new artifact version...")
            _               (p/build-jar config)
